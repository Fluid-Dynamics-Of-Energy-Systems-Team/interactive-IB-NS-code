...
public:
	...
	void readIB(string fname) {
		...
		else { cout << "ERROR: FILE NOT FOUND: " << fname << endl; exit(-1);}
	}
...
public:
	...
	void init() {
		...
		intersectIBwMesh();
		if (pts.size() > 2)
			if (fabs(mag(pts[0] - pts[pts.size()-1])) < 1.0e-2)
				tesselate();
	}
...
//					nvec.print();
	...
	void tesselate() {
		...
		int vert = 1;
		for (int i=0; i<nPts*2; i=i+2)
		{
			in.segmentlist[i]   = vert;
			in.segmentlist[i+1] = vert+1;
			vert++;
		}
		in.segmentlist[2*nPts-1] = 1; // connect last point to first
