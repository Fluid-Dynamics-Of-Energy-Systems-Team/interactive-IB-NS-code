...
//			vInlet = 0.0;
	...
	case '2':
		{
			ibObj.clear();
			IBObject obj = IBObject("objects/nozzle.txt", dx, imax, jmax);
	...
	case '3':
		{
			ibObj.clear();
			IBObject obj = IBObject("objects/circle.txt", dx, imax, jmax);
			obj.scale(0.5);
			ibObj.push_back(obj);
			vInlet = 0.0;
		}
	...
	case '8':
		{
			...
			obj = IBObject("objects/circle.txt", dx, imax, jmax);
			obj.scale(0.4);
